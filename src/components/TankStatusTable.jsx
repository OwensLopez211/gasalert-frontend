...
const TankStatusTable = () => {
  ...
  useEffect(() => {
    const fetchTanks = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8000/api/tanks?estacion_id=${estacionId}`, {
          headers: {
        ...
        const capacity = tanksData.reduce((acc, tank) => acc + tank.capacidad_total, 0);
        const lowAlerts = tanksData.filter(tank => tank.estado === 'bajo').length;
        const criticalAlerts = tanksData.filter(tank => tank.estado === 'critico').length;
  ...
  return (
    <div className="bg-[#111517] p-6 rounded-lg border border-gray-700">
      <h2 className="text-white text-lg font-bold mb-4">Fuel Tanks</h2>

      {/* Resumen general */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-800 p-4 rounded-lg text-center">
          <p className="text-gray-400 text-sm">Total Fuel Capacity</p>
          <p className="text-white text-2xl font-bold">
            {totalCapacity.toLocaleString('es-ES', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} Litros
          </p>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg text-center">
      ...
      <h3 className="text-white text-md font-semibold mb-4">Real-time Monitoring</h3>
        ...
        <tbody>
          {tanks.map((tank, index) => (
            <tr key={index} className="border-b border-gray-700">
                ...
                <div className="flex items-center">
                  <div className="h-2 w-full bg-gray-600 rounded-full mr-2">
                    <div
                      ...
                      style={{ width: `${tank.ultima_lectura?.nivel || 0}%` }}
                    ></div>
                  </div>
                  <span>
                    {tank.ultima_lectura 
                      ? `${tank.ultima_lectura.volumen.toLocaleString('es-ES', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} L (${tank.ultima_lectura.nivel.toFixed(1).replace('.', ',')}%)`
                      : 'Sin datos'}
                  </span>
                </div>
              </td>
              <td className="py-3 px-4">
                {tank.capacidad_total.toLocaleString('es-ES', {maximumFractionDigits: 1})} Litros
              </td>
              <td className="py-3 px-4">
                <span
              ...
              <td className="py-3 px-4">{tank.ultima_lectura?.fecha ? new Date(tank.ultima_lectura.fecha).toLocaleString('es-ES') : 'Sin datos'}</td>
            </tr>
          ))}
        </tbody>
      </table>
